    ...
    def dry_run(
        ...
        for file_path in md_files:
            try:
                ...
                if whole_word:
                    pattern = (
                        r"(?<![a-zA-Z0-9_])" + re.escape(search) + r"(?![a-zA-Z0-9_])"
                    )
        ...
        for file_path in md_files:
            try:
                ...
                if whole_word:
                    ...
                    flags = re.IGNORECASE if case_insensitive else 0
                    new_content = re.sub(
                        pattern, lambda m: replacement, original_content, flags=flags
                    )
                    ...
                    count = len(
                        re.findall(re.escape(search), original_content, re.IGNORECASE)
                    )
                else:
...
Examples:
  ...
  python md_replace.py ./docs "OLD" "new" --case-insensitive
    ...
    parser.add_argument(
        "--case-insensitive", action="store_true", help="Case-insensitive replacement"
    )
    parser.add_argument(
        "--whole-word",
        action="store_true",
        help="Match whole words only (e.g., 'Var' won't match 'Variance')",
    )
    parser.add_argument("--undo", action="store_true", help="Undo the last operation")
    parser.add_argument(
        "--list-backups", action="store_true", help="List all backup operations"
    )
    ...
    try:
        ...
        if not args.search or not args.replacement:
            parser.error(
                "search and replacement strings are required (unless using --undo or --list-backups)"
            )
        ...
        files_modified, total_replacements, files_skipped = (
